...
for arg in sys.argv[1:]:
    if '=' not in arg:
        ...
        # Security: Instead of exec(), parse the file as a safe dictionary of constants
                    ...
                    if isinstance(target, ast.Name):
                        try:
                            ...
                            if target.id in globals():
                                assert type(value) == type(globals()[target.id])
                                print(f"Overriding from file: {target.id} = {value}")
