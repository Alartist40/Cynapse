...
class ElaraModelLoader:
    ...
    def load(
        ...
        if not self.model_path.exists():
            print(f"Model not found: {self.model_path}. Using random weights for testing.", file=sys.stderr)
        ...
        self.config = GPTConfig(
            block_size=1024,
            vocab_size=50304,
            n_layer=12,    # Reduced layers from 32 to 12 for sandbox stability
            n_head=12,
            n_embd=768,    # GPT-2 equivalent size for functional testing
            dropout=0.0,
            bias=False,
            num_experts=8,
            moe_top_k=2
        ...
        print(f"Loading Elara model...", file=sys.stderr)
        ...
        try:
            ...
            if hasattr(torch, 'compile') and device == 'cuda':
                print("  Compiling model (PyTorch 2.0+)...", file=sys.stderr)
