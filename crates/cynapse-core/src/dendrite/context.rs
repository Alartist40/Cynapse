...
use crate::dendrite::graph::{Node, NodeType, Dendrite};
...
impl DendriteContext {
    ...
    pub fn build_prompt(&self, user_message: &str, max_tokens: usize) -> String {
        ...
        // Message-specific context always recomputes.
        if !user_message.trim().is_empty() {
            inner.dirty = false; // invalidate stale cache after this turn
        ...
        if let Ok(ids) = store.fts_search(user_message, 20) {
            for id in ids {
                if let Some(n) = inner.graph.get(&id) {
                    ...
                    for neighbor in inner.graph.neighbors(&n.id) {
                        add_node(&neighbor, &mut out, &mut seen);
                    }
                }
            }
        }
...
/// Rough token estimate (1 token ≈ 4 chars), matching the Go original.
