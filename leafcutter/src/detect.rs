...
/// Board / SoC hardware model classification.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BoardKind {
    Unknown,
    OrangePi6Plus,
...
impl BoardKind {
    pub fn label(self) -> &'static str {
        match self {
            BoardKind::Unknown => "Generic Host",
            BoardKind::OrangePi6Plus => "Orange Pi 6 Plus (CIX P1 SoC — 12-Core ARMv9 + Mali-G720 Vulkan GPU + 30 TOPS Zhouyi AIPU)",
            BoardKind::GenericSbc => "Generic SBC",
        }
    }
}
...
pub fn choose_tier(
    ...
    let (_total_mb, avail_mb) = meminfo_mb();
    let avail_ram_mb = if avail_mb > 0 { avail_mb } else { ram_available_mb };
    let avail_ram_bytes = avail_ram_mb.saturating_mul(1024 * 1024);
    // Reserve 256 MB headroom for OS + KV cache + activations (llama.cpp efficient mmap overhead)
    const RESERVE_BYTES: u64 = 256 * 1024 * 1024;
    let need = model_size_bytes.saturating_add(RESERVE_BYTES);
    if need <= avail_ram_bytes {
        Tier::FastCpu
    } else {
        Tier::StreamingCpu
    }
...
/// Cheap, dependency-free board / SoC probe.
fn probe_board() -> BoardKind {
        ...
        .or_else(|_| std::fs::read_to_string("/proc/device-tree/model"))
    {
        ...
        if m.contains("orange pi 6 plus") || m.contains("cix") || m.contains("p1") {
            return BoardKind::OrangePi6Plus;
        }
    }
