...
enum Commands {
    ...
    /// Start the standalone Ollama-replacement HTTP API server daemon (serves /api/* and /v1/*)
    Serve {
        /// Path to GGUF model file (optional, resolved dynamically by ModelScheduler)
        #[arg(short, long, default_value = "")]
        model: String,
        /// HTTP port to listen on (default: 11434 Ollama port)
    ...
    /// Run system diagnostic health check (alias: 'leaf doctor')
    Doctor {
...
fn scan_models(dirs: &[PathBuf]) -> Vec<(PathBuf, u64)> {
    ...
    models.sort_by(|a, b| {
        let a_is_gguf = a.0.extension().and_then(|s| s.to_str()) == Some("gguf");
        let b_is_gguf = b.0.extension().and_then(|s| s.to_str()) == Some("gguf");
        if a_is_gguf != b_is_gguf {
            b_is_gguf.cmp(&a_is_gguf)
        } else {
...
fn cmd_run_safetensor(model_arg: &str, mut temp: f32, _top_p: f32, mut max_tokens: usize) {
    ...
    loop {
        ...
        match result {
            ...
            Err(e) => {
                eprintln!("⚠️  [safetensor backend error] {}", e);
                eprintln!("💡 Note: Unquantized FP16/BF16 Safetensors models (>50B parameters) require 80–120 GB RAM in PyTorch.");
                eprintln!("   To run massive models like DeepSeek v4 Flash 2 on low-RAM hardware (8–16 GB), convert to GGUF format (Q4_K_M or Q2_K) for Leafcutter's native Tier 3 streaming CPU engine:");
                eprintln!("   python llama.cpp/convert_hf_to_gguf.py {}", model_dir);
                continue;
            }
        }
    }
...
fn cmd_doctor(auto_fix: bool) {
    use leafcutter::detect::{choose_tier, probe_hardware, Tier};
    ...
    println!("{}", "╔══════════════════════════════════════════════════════════════╗".purple().bold());
    println!("{}", "║    🌿 LEAFCUTTER LLM ENGINE — AUTONOMOUS CS DOCTOR & REPAIR  ║".yellow().bold());
    println!("{}", "╚══════════════════════════════════════════════════════════════╝".purple().bold());
    println!();
    ...
    // 1. Host Hardware & CPU Architecture
    let hw = probe_hardware();
    println!("{}", purple("  💻 System Hardware & CPU Architecture:"));
    println!("     • Board Hardware:     {}", gold(hw.board.label()));
    println!("     • OS Target:          {}", gold(std::env::consts::OS));
    println!("     • Architecture:       {}", gold(std::env::consts::ARCH));
    println!("     • Total System RAM:   {}", gold(&format!("{} MB", hw.ram_total_mb)));
    println!("     • Available Free RAM: {}", gold(&format!("{} MB", hw.ram_available_mb)));

    let logical_cpus = std::thread::available_parallelism().map(|u| u.get()).unwrap_or(1);
    let worker_threads = leafcutter::init::effective_thread_count();
    println!(
        "     • CPU Cores:          {} logical (Rayon worker pool: {} threads)",
        gold(&logical_cpus.to_string()),
        gold(&worker_threads.to_string())
    );
    println!("     • GPU Accelerator:    {:?}", hw.gpu);
    println!("     • NPU Driver Node:    {:?}", hw.npu);
    println!();

    // 2. Binary Installation & PATH
    println!("{}", purple("  🛠  Binary Installation & PATH Status:"));
    let exe_path = std::env::current_exe().unwrap_or_default();
    println!("     • Current Executable: {}", gold(&exe_path.display().to_string()));

    let in_local_bin = exe_path.to_string_lossy().contains(".local/bin");
    let leaf_symlink_ok = if let Ok(home) = std::env::var("HOME") {
        let symlink = std::path::PathBuf::from(home).join(".local/bin/leaf");
        symlink.exists()
    } else {
        false
    };
    println!("     • Installed in PATH:   {}", if in_local_bin { "YES (OK)".green().bold() } else { "NO (~/.local/bin recommended)".yellow().bold() });
    println!("     • 'leaf' Shortcut:     {}", if leaf_symlink_ok { "INSTALLED (OK)".green().bold() } else { "NOT FOUND (run install.sh)".yellow().bold() });
    
    if (!in_local_bin || !leaf_symlink_ok) && auto_fix {
        if let Ok(home) = std::env::var("HOME") {
            let bin_dir = std::path::PathBuf::from(home).join(".local/bin");
            let _ = std::fs::create_dir_all(&bin_dir);
            let target_leafcutter = bin_dir.join("leafcutter");
            let target_leaf = bin_dir.join("leaf");
            if exe_path.exists() && exe_path != target_leafcutter {
                let _ = std::fs::copy(&exe_path, &target_leafcutter);
            }
            let _ = std::fs::remove_file(&target_leaf);
            #[cfg(unix)]
            let _ = std::os::unix::fs::symlink(&target_leafcutter, &target_leaf);
            fixes_applied.push("Installed/re-linked ~/.local/bin/leafcutter and ~/.local/bin/leaf shortcut".to_string());
        }
    } else if !in_local_bin {
        issues.push("Leafcutter binary is not running from ~/.local/bin".to_string());
    }
    println!();

    // 3. Model Discovery & Tier Classification
    println!("{}", purple("  🧠 Model Registry & Cache Tiering:"));
    let dirs = resolve_models_dirs();
    let models = scan_models(&dirs);
    let mut smallest_gguf: Option<PathBuf> = None;

    if models.is_empty() {
        println!("     ⚠️  No models found in search paths:");
        for d in &dirs {
            println!("        - {}", d.display());
        }
        if auto_fix {
            if let Ok(home) = std::env::var("HOME") {
                let default_dir = std::path::PathBuf::from(home).join("Downloads/models");
                let _ = std::fs::create_dir_all(&default_dir);
                let _ = leafcutter::config::add_model_dir(&default_dir.to_string_lossy());
                fixes_applied.push(format!("Created and registered default model directory at {}", default_dir.display()));
            }
        } else {
            println!("     💡 Add a directory with: leafcutter source add <dir>");
            issues.push("No models found in search paths".to_string());
        }
    } else {
        println!("     Found {} available model(s):", gold(&models.len().to_string()));
        for (path, size_bytes) in models.iter().take(10) {
            let name = path.file_name().and_then(|n| n.to_str()).unwrap_or("model");
            let size_mb = (*size_bytes / (1024 * 1024)) as u64;
            let tier = choose_tier(hw.gpu, hw.ram_available_mb, *size_bytes, false);
            let tier_str = match tier {
                Tier::Gpu => "Tier 1 (GPU)",
                Tier::FastCpu => "Tier 2 (Fast CPU / RAM Cached)",
                Tier::StreamingCpu => "Tier 3 (Streaming CPU / mmap)",
                Tier::Unsupported => "Tier Unsupported (RAM Limit)",
            };
            println!("     • {:<35} {:>7.2} GiB -> {}", gold(name), size_mb as f64 / 1024.0, purple(tier_str));

            if path.extension().and_then(|s| s.to_str()) == Some("gguf") {
                if smallest_gguf.is_none() || *size_bytes < std::fs::metadata(smallest_gguf.as_ref().unwrap()).map(|m| m.len()).unwrap_or(u64::MAX) {
                    smallest_gguf = Some(path.clone());
                }
            }
        }
    }
    ...
    // 4. Active Runtime Inference Dry-Run Test
    println!("{}", purple("  🧪 Active Runtime Inference Test:"));
    if let Some(target) = smallest_gguf {
        let name = target.file_name().and_then(|n| n.to_str()).unwrap_or("model");
        print!("     • Dry-running 1-token forward pass on '{}' ... ", name);
        let start = std::time::Instant::now();
        match Engine::load(&target.to_string_lossy()) {
            Ok(mut engine) => {
                let prompt_tokens = engine.tokenize("hi", true);
                let out_tokens = engine.generate(&prompt_tokens, 1, 0.7, 0.95);
                let text = engine.decode(&out_tokens);
                let dur = start.elapsed();
                println!(
                    "{}",
                    format!("PASSED ({} tok in {:.1}ms, output: {:?})", out_tokens.len(), dur.as_secs_f64() * 1000.0, text.trim()).green().bold()
                );
            }
            Err(e) => {
                println!("{}", format!("FAILED TO LOAD ({})", e).red().bold());
                issues.push(format!("Model load failed on {}: {}", name, e));
            }
        }
    } else {
        println!("     ⚠️  Skipped (No .gguf model available to test)");
    }
    println!();

    // 5. Ollama Standalone Replacement Server Check
    println!("{}", purple("  🌐 Ollama Standalone Replacement Daemon:"));
    let server_online = std::net::TcpStream::connect("127.0.0.1:11434").is_ok();
    if server_online {
        println!("     • Daemon Port 11434:   {}", "ONLINE (http://localhost:11434)".green().bold());
        println!("     • Wire Protocols:      Ollama /api/* (NDJSON) + OpenAI /v1/* (SSE)");
    } else {
        println!("     • Daemon Port 11434:   {}", "STANDBY (Start daemon with: leaf serve)".yellow().bold());
        println!("     • Wire Protocols:      Ollama /api/* (NDJSON) + OpenAI /v1/* (SSE)");
    }
    println!("     • Memory Scheduler:    ModelScheduler (5-min idle keep_alive auto-unload)");
    println!();

    // 6. Cynapse Integration Check
    println!("{}", purple("  ⚡ Cynapse Frontend Integration:"));
    let cynapse_path = leafcutter::launch::find_binary("cynapse");
    match cynapse_path {
        Some(p) => println!("     • Cynapse Binary:      ONLINE ({})", gold(&p.display().to_string())),
        None => {
            println!("     • Cynapse Binary:      NOT INSTALLED (Build in ./Documents/portfolio/cynapse)");
            if auto_fix {
                let cynapse_repo = std::path::PathBuf::from("/home/orangepi/Documents/portfolio/cynapse");
                if cynapse_repo.exists() {
                    print!("       🔨 Building Cynapse release binary... ");
                    let status = std::process::Command::new("cargo")
                        .args(["build", "--release"])
                        .current_dir(&cynapse_repo)
                        .stdout(std::process::Stdio::null())
                        .stderr(std::process::Stdio::null())
                        .status();
                    if status.map(|s| s.success()).unwrap_or(false) {
                        let built = cynapse_repo.join("target/release/cynapse");
                        if let Ok(home) = std::env::var("HOME") {
                            let dest = std::path::PathBuf::from(home).join(".local/bin/cynapse");
                            let _ = std::fs::copy(built, dest);
                            println!("{}", "BUILT & INSTALLED".green().bold());
                            fixes_applied.push("Compiled and installed Cynapse release binary to ~/.local/bin/cynapse".to_string());
                        }
                    } else {
                        println!("{}", "BUILD FAILED".red().bold());
                        issues.push("Cynapse cargo build failed during auto-repair".to_string());
                    }
                }
            } else {
                issues.push("Cynapse binary is not installed in PATH".to_string());
            }
        }
    }
    println!();
    ...
    if !fixes_applied.is_empty() {
        println!("{}", "🔧 Autonomous Doctor Fixes Applied:".green().bold());
    ...
    if issues.is_empty() {
        println!("{}", "✅ Leafcutter system doctor check complete: ALL CHECKS PASSED.".green().bold());
    } else {
        println!("{}", "⚠️  Leafcutter system doctor detected issues:".yellow().bold());
        for issue in &issues {
            println!("     ❌ {}", issue.red());
        }
        if !auto_fix {
            println!("     💡 Run 'leaf doctor --fix' to automatically repair detected issues!");
        }
    }
}
...
fn cmd_run(model_arg: &str, mut temp: f32, mut top_p: f32, mut max_tokens: usize) {
    ...
    let file_basename = path.file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_else(|| path_str.clone());
    eprintln!("🌿 Selected model: {}", gold(&file_basename));
    ...
    loop {
        ...
        let readline = if let Some(ref mut editor) = rl {
            editor.readline(&prompt_str)
        } else {
            print!("{}", prompt_str);
            io::stdout().flush().ok();
...
fn resolve_model_arg(model_arg: &str) -> PathBuf {
    ...
    if let Ok(idx) = model_arg.parse::<usize>() {
        if idx < models.len() {
            return models[idx].0.clone();
        }
    }

    for (p, _) in &models {
        if let Some(name) = p.file_name().and_then(|n| n.to_str()) {
            if name.to_lowercase() == model_arg.to_lowercase() {
                return p.clone();
            }
        }
    }
    ...
    for (p, _) in &models {
        if let Some(name) = p.file_name().and_then(|n| n.to_str()) {
            if name.to_lowercase().contains(&model_arg.to_lowercase()) {
                return p.clone();
            }
        }
    }
...
async fn cmd_serve(model_path: &str, port: u16, host: &str, engine_type: &str, benchmark: bool) {
    let resolved = if model_path.is_empty() {
        let dirs = resolve_models_dirs();
        let models = scan_models(&dirs);
        if models.is_empty() {
    ...
    let model_str = resolved.to_string_lossy().to_string();

    #[cfg(feature = "llama-ffi")]
    {
        run_server_ffi(&model_str, port, host, engine_type, benchmark).await;
    }
    #[cfg(not(feature = "llama-ffi"))]
    {
        ...
        run_server_native(&model_str, port, host).await;
    }
}

#[cfg(not(feature = "llama-ffi"))]
async fn run_server_native(_model_path: &str, port: u16, host: &str) {
    use leafcutter::api::run_server;

    let port = if port == 0 || port == 8080 { 11434 } else { port };

    eprintln!("{}", gold("🌿 LeafcutterLLM — Standalone Ollama Replacement Daemon"));
    eprintln!("   {}   http://{}:{}", dim_purple("Address:"), host, port);
    eprintln!("   {}   /api/* (NDJSON) + /v1/* (OpenAI SSE)", dim_purple("Endpoints:"));
    eprintln!();

    run_server(None, port, host).await;
}
