...
/// Check if the recent token sequence contains an N-token loop repeated 3 times.
fn is_in_repetition_loop(generated: &[usize]) -> bool {
    let n = generated.len();
    if n < 12 {
        return false;
    }
    for len in 2..=16 {
        if n >= len * 3 {
            let slice1 = &generated[n - len..];
            let slice2 = &generated[n - 2 * len..n - len];
            let slice3 = &generated[n - 3 * len..n - 2 * len];
...
/// Check if accumulated generation text contains any control or section stop markers.
fn contains_stop_marker(text: &str) -> bool {
    let markers = [
        "<|im_end|>",
        "<|im_start|>",
        "<|endoftext|>",
...
impl Engine {
        ...
        F: FnMut(usize, &str) -> bool,
    {
        ...
        let is_stop = next_token == self.config.eos_token || stop_tokens.contains(&next_token);
        if is_stop {
            return generated;
        }

        let mut wrapped_cb = |id: usize, piece: &str| -> bool {
            accumulated_text.push_str(piece);
            if contains_stop_marker(&accumulated_text) {
                return false;
            }
            on_token(id, piece)
        };

        if !self.emit_stream_token(next_token, &mut pending_bytes, &mut wrapped_cb) {
            return generated;
        }
        ...
        for _ in 0..max_tokens - 1 {
            ...
            if recent_tokens.len() > 64 {
                recent_tokens.drain(0..recent_tokens.len() - 64);
            }
            apply_repeat_penalty(&mut logits, &recent_tokens, 1.15);
            ...
            let is_stop = next_token == self.config.eos_token
                || stop_tokens.contains(&next_token)
                || is_in_repetition_loop(&generated);
            if is_stop {
                break;
            }
    ...
    F: FnMut(usize, &str) -> bool,
{
    ...
    if text.contains("<|im_end|>") || text.contains("<|endoftext|>") || text.contains("<|im_start|>") {
        let clean = text
            .replace("<|im_end|>", "")
            .replace("<|endoftext|>", "")
            .replace("<|im_start|>", "");
