...
impl GGUFModel {
    ...
    fn compute_cache_budget_bytes(total_model_bytes: usize, config: &ModelConfig) -> usize {
        ...
        let ram_mb = if avail_mb > 0 { avail_mb } else { total_mb };
        if ram_mb == 0 {
            return 0; // meminfo unavailable → unlimited (backward compatible)
        }

        // Fast path: if total model size + 256 MB headroom fits in CURRENTLY AVAILABLE free RAM,
        // cache all layers residently in memory (return 0 = unlimited cache budget).
        let headroom_bytes = 256 * 1024 * 1024;
        ...
        for layer_idx in 0..sample_layers {
            ...
            for (gguf_suffix, _engine_name) in mappings.iter() {
                ...
                if self.file.get_tensor_info(&name).is_none() {
                    ...
                    if matches!(
                        self.architecture,
                        ModelArchitecture::Qwen35 | ModelArchitecture::Qwen36 | ModelArchitecture::DeepSeek2
                    ) {
                        ...
                        // Qwen3.6 and DeepSeek2 MoE layers lack the dense ffn_* projections
                        // (they use ffn_*_exps for routed experts instead)
                        if self.architecture == ModelArchitecture::Qwen36 || self.architecture == ModelArchitecture::DeepSeek2 {
                            if *gguf_suffix == "attn_q_norm.weight" || *gguf_suffix == "attn_k_norm.weight" {
                                continue;
                            }
                            ...
                            if !is_moe_layer
                                && (gguf_suffix.contains("_exps")
                                    || gguf_suffix.contains("_shexp")
                                    || gguf_suffix.contains("ffn_gate_inp"))
                            {
                                continue;
                            }
