    ...
    F: FnMut(&str, bool) -> bool,
{
    ...
    let mut script_candidates = vec![
        std::path::PathBuf::from("scripts/leafcutter_safetensor_run.py"),
        std::path::PathBuf::from("./scripts/leafcutter_safetensor_run.py"),
        std::path::PathBuf::from("../scripts/leafcutter_safetensor_run.py"),
        std::path::PathBuf::from(format!("{}/Documents/portfolio/LeafcutterLLM/rust/scripts/leafcutter_safetensor_run.py", home)),
        std::path::PathBuf::from(format!("{}/.local/share/leafcutter/scripts/leafcutter_safetensor_run.py", home)),
    ];
    if let Some(dir) = exe_dir {
        script_candidates.push(dir.join("scripts/leafcutter_safetensor_run.py"));
        script_candidates.push(dir.join("../scripts/leafcutter_safetensor_run.py"));
    }
